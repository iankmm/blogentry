...
So we just create a contract that self-destructs and sends the funds to our address, right? However, it turns out that the contract performs some sanity checks of the implementation address.
...
0x33FF
```
`0x33` loads the caller's address, and `0xFF` represents the `selfdestruct` opcode. Hence, making the contract self-destruct and drain its balance to the caller.  However, now I needed to make the contract address have 12 leading zeros.
...
Amusingly, the price of 0.1 ETH was worth around 350 dollars the day I solved the challenge, but now it's worth around 270 dollars. Not sure whether to laugh or cry here, considering my ETH bags. Anyway, this was a fun challenge. Big thanks to Kaden for putting it together.